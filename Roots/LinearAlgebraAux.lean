...
theorem Module.coe_end_one {k V : Type _} [Semiring k] [AddCommMonoid V] [Module k V] :
    ⇑((1 : (Module.End k V)ˣ) : Module.End k V) = id :=
  rfl
...
theorem toPreSymmetry_sq {x : V} {f : Dual k V} (h : f x = 2) :
  ...
  ext β
  rw [LinearMap.pow_apply, iterate_succ, iterate_one, comp_apply]
  -- simp only [toPreSymmetry_apply, map_sub, map_smul, LinearMap.id_coe, id_eq]
  nth_rw 1 [toPreSymmetry_apply]
